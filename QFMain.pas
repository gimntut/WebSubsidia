...
//  end;
  ...
  private
    ...
    procedure SolveF7(DateV: TDate; var LastOtvet: Extended; var Otvet: Extended; out Polusch_SostFam:string);
...
uses Publ, PublFile, ShellAPI, ClipBrd, Printers, QFLoadList, ShFolder, Registry,
  ...
  cdbf, Math, uTime, uImportBases, uFieldsDlg;
{$R *.dfm}
...
procedure TForm9.SolveF7(DateV: TDate; var LastOtvet: Extended; var Otvet: Extended; out Polusch_SostFam:string);
var
  I: Integer;
  DV: TDate;
  SposV: Integer;
...
//  ShowMessage(Chpr3.Text);
  ...
  begin
    ...
    Polusch_SostFam := format('%2s/%-2s',[PublStr.Trim(Chpr3.ValueByName[I, 'POLUCH']),PublStr.Trim(Chpr3.ValueByName[I, 'SOS_FAM'])]);
    LastOtvet := Otvet;
    LPS:=Polusch_SostFam;
    SposV := StrToInt(Chpr3.ValueByName[I, 'SV']);
    if SposV in [2,5] then break;
  end;
  if math.isNan(Otvet) then begin
    Otvet := LastOtvet;
    Polusch_SostFam := LPS;
  end;
end;
...
procedure TForm9.ToolButton12Click(Sender: TObject);
begin
  FieldsDlg.InCHPR:=Chpr;
  if FieldsDlg.Execute then begin
    Chpr.ShowFields.CommaText:=FieldsDlg.OutList;
    OutTable(MessageStr);
  end;
end;
...
const
  ...
  + #13#10'|----|---------|-----|-------------|-------------|----------------|--------|--------|--------|--------|--------|--------|--------|--------|'
  + #13#10'| №  | Месяц,  |Сос- | Размер      |  Размер     | В том числе    |        |        | Элект- | Водо-  |        | ИТОГО  |        |        |'
  + #13#10'|    | год     |тав  | субсидии    |  выплаченой | доплачено,     |  ЖКУ   |  Газ   | роэнер-| снаб-  | Прочее |  ком.  | Удер-  |  Доп-  |'
  + #13#10'|    |         |семьи| по формуле  |  субсидии   | удержено       |        |        |  гия   | жение  |        | услуги | жание  |  лата  |'
  + #13#10'|    |         |     |             |             |                |        |        |        |        |        |        |        |        |'
  + #13#10'|----|---------|-----|-------------|-------------|----------------|--------|--------|--------|--------|--------|--------|--------|--------|';
                                                                                                                                             
  Hvost:string =                                                                                                                             
    #13#10'==========================================================================================================================================='
  + #13#10'|   РАСЧЁТЫ    |_____|_____________|_____________|________________|________|________|________|________|________|________|________|________|'
  + #13#10'|              |     |             |             |                |        |        |        |        |        |        |        |        |'
  + #13#10'==========================================================================================================================================='
  + #13#10''
...
begin
  ...
  begin
    DateVipl := ChprTmp.Values[I][1];
    DateV := StrToDate(DateVipl);
    SolveF7(DateV, LastOtvet, Otvet, poluch_sostav);
    SumVipl := ChprTmp.Values[I][2];
    SV:=StrToCurr(AnsiReplaceStr(Sumvipl,'.',DecimalSeparator));
    Dopl:=StrToCurr(AnsiReplaceStr(ChprTmp.Values[I][15],'.',DecimalSeparator));
    SumDopl:='';
    ...
    Sum:=Sum+SV;
    Month := format('%s.%s',[ToZeroStr(MonthOf(DateV),2),IntToStr(YearOf(DateV))]);
    ResultStr := ResultStr + format(#13#10'| %2d | %6s |%s| %11.2f | %11s | %14s |        |        |        |        |        |        |        |        |',[I+1,Month,poluch_sostav,Otvet,SumVipl,SumDopl]);
    if I=ChprTmp.Count-2
    then
