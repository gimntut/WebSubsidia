...
type
  ...
  //
  TScriptForm = class(TForm, IActiveScriptSite, IActiveScriptSiteWindow)
...
implementation
uses ComObj, Variants, errfrm;

{ TScriptForm }

procedure TScriptForm.CloseScriptEngine;
begin
  FParser := nil;
...
procedure TScriptForm.CreateScriptEngine(Language: TScriptLanguage);
begin
  CloseScriptEngine;
...
function TScriptForm.EnableModeless(fEnable: BOOL): HResult;
begin
  Result := S_OK;
end;
...
function TScriptForm.GetDocVersionString(out pbstrVersion: WideString): HResult;
begin
  Result := E_NOTIMPL;
end;

function TScriptForm.GetItemInfo(pstrName: LPCOLESTR; dwReturnMask: DWORD;
  out ppiunkItem: IInterface; out ppti: ITypeInfo): HResult;
begin
  Result := E_NOTIMPL;
end;

function TScriptForm.GetLCID(out plcid: LCID): HResult;
begin
  plcid := GetSystemDefaultLCID;
  Result := S_OK;
end;

function TScriptForm.GetWindow(out phwnd: HWND): HResult;
begin
  phwnd := Handle;
  Result := S_OK;
end;

function TScriptForm.OnEnterScript: HResult;
begin
  Result := S_OK;
end;

function TScriptForm.OnLeaveScript: HResult;
begin
  Result := S_OK;
end;

function TScriptForm.OnScriptError(
...
function TScriptForm.OnScriptTerminate(var pvarResult: OleVariant;
  var pexcepinfo: EXCEPINFO): HResult;
begin
  Result := S_OK;
end;

function TScriptForm.OnStateChange(ssScriptState: SCRIPTSTATE): HResult;
begin
  Result := S_OK;
end;
