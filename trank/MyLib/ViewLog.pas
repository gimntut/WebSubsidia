...
uses
  ...
  Dialogs, ExtCtrls, StdCtrls, ImgList, ComCtrls, ToolWin, publStr, DB, Menus;
type
...
procedure LogLock;
procedure LogUnLock;
procedure LogObjectAsText(Component:TComponent);
...
begin
  ...
  SetLength(S,Stream2.size);
  Stream2.Position:=0;
  Stream2.Read(S[1],Stream2.size);
  Stream2.Free;
  Log(S);
end;
...
procedure TLogWindow.FormDestroy(Sender: TObject);
begin
