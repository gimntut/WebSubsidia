...
uses
  Windows, Messages, SysUtils, Classes, Controls, StdCtrls, Menus, PublStr,
  Grids, ExtCtrls, Graphics, Forms, DateUtils, Publ, HardPubl;
type
  ...
  private
   ...
   FOnEditShow: TEditEvent;
   function DoEdit(ARow:integer):boolean;
   function DoEditShow(ARow:integer):boolean;
  ...
  published
   ...
   property OnEditShow:TEditEvent read FOnEditShow write FOnEditShow;
  ...
  private
   ...
   function GetComboCol(ACol: integer): TStrings;
   ...
   function GetIsCombo(ACol: integer): Boolean;
   function GetValuesInt(ACol, ARow: integer): integer;
   procedure SetValuesInt(ACol, ARow: integer; const Value: integer);
  protected
   ...
   property IsCombo[ACol:integer]:Boolean read GetIsCombo;
  public
   Exiting:boolean;
   Constructor Create(AOwner:TComponent); override;
   Destructor Destroy; override;
   function Focused: Boolean; override;
   function SwitchValueTo(ACol,ARow:Integer; Value:TObject):Boolean;
   procedure Clear;
   ...
   property ComboCol[ACol:integer]:TStrings read GetComboCol write SetComboCol;
   ...
   property ValuesInt[ACol,ARow:integer]:integer read GetValuesInt write SetValuesInt;
  published
...
function TNewGrid.GetComboCol(ACol: integer): TStrings;
begin
 result:=FColumns[ACol].Strings;
end;
...
// OldItemIndex:=ItemIndex[btCol,btRow];
 IsNew:=false;
 With List do Begin
  Visible:=false;
...
begin
 List.Visible:=false;
 if not IsNew
 then ToCell(btCol,btRow,OldCell,OldData)
 else SetEditText(btCol,btRow,Cells[btCol,btRow]);
 EditorMode:=false;
 Form:=GetParentForm(self);
...
begin
 Case ShortCut(Key,Shift) of
  scAlt+vk_Down: Begin
   if BeMark and IsCombo[Col] then BtnClick(DownBtn);
   Exit;
  End;
  else inherited;
 End;
...
begin
 ...
 With List, Items do ToCell(btCol,btRow,ItemIndex);
end;
...
begin
 result:=true;
 if Columns[Col]<>nil then result:=Columns[Col].DoEditShow(Row);
...
begin
 ...
 FBeMark := Value;
 DownBtn.Visible:=IsCombo[Col] and Value;
end;
...
function TNewGrid.GetIsCombo(ACol: integer): Boolean;
begin
 Result:=Columns[ACol].Strings.Count>0;
end;
...
function TNewGrid.GetValuesInt(ACol, ARow: integer): integer;
begin
 Result:=Str2Int(Cells[ACol,ARow]);
end;
...
procedure TNewGrid.SetValuesInt(ACol, ARow: integer; const Value: integer);
begin
 Cells[ACol,ARow]:=Int2Str(Value);
end;

function TNewGrid.SwitchValueTo(ACol,ARow:Integer; Value: TObject):boolean;
Var
 n:integer;
begin
 result:=false;
 if (ACol<0) or (ACol>ColCount-1) then EXit;
 n:=ComboCol[ACol].IndexOfObject(Value);
 if n=-1 then Exit;
...
begin
 ...
 else if IsCombo[Col] and Focused then BtnClick(DownBtn);
end;
...
function TNGColumn.DoEditShow(ARow: integer): boolean;
begin
 Result:=not FReadOnly and not Grid.OnlyView;
 if Assigned(FOnEditShow) then FOnEditShow(Grid,ARow,Result)
end;
...
begin
 if IsCombo[Col] and not OnlyView and (Message.CharCode>27) then Begin
  BtnClick(DownBtn);
