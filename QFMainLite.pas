...
begin
  ...
  if PrintDialog1.Execute then begin
    ...
    then begin
      ...
      if edSearchLine.Text<>'' then
        sts.Insert(0,format('По поисковому запросу "%s" найдено %d из %d записей:',[edSearchLine.Text,Chpr.FilterCount-1,Chpr.Count-1]));
        sts.Insert(1,'');
    end;

    if Pos('EPSON',AnsiUpperCase(Printer.Printers[Printer.PrinterIndex]))>0 then begin
      PrintToEpson(Sts);
    end else PrintToLaser2(Sts,'Справка',Memo1.Font.Name);
  end;
  sts.Free;
end;
